(function() {
  ...
  function handleMouseMove(e) {
    ...
    // Always remove previous highlight before processing new target
    if (currentHighlight) {
      currentHighlight.classList.remove('inspector-highlight');
    }
    
    if (!target || target === document.body || target === document.documentElement || target === inspectorStyle) {
      // Mouse moved off an element or onto body/html/inspector's own style.
      // Clear currentHighlight and send leave message.
    ...
    if (active) {
      ...
      if (!inspectorStyle) {
        ...
        inspectorStyle.textContent = `
          ...
          /* Ensure our inspector elements don't get a crosshair cursor */
          .inspector-active body > style.inspector-style,
          .inspector-active #inspector-overlay {
        ...
        `;
        inspectorStyle.classList.add('inspector-style'); // Add a class for potential CSS exclusion
      ...
      // Send a final leave message when deactivating to ensure parent UI cleans up
      window.parent.postMessage({
        type: 'INSPECTOR_LEAVE'
      }, '*');
