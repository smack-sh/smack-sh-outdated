...
export function simplifysmackActions(input: string): string {
  // Using regex to match smackAction tags that have type="file"
  const regex = /(<smackAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/smackAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<smackAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</smackAction>`;
    });

  return `<smackArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</smackArtifact>`;
}
