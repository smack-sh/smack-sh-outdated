...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifysmackActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifysmackActions(content);

      content = content.replace(/<div class=\\"__smackThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`smack failed to select files`);
  }
