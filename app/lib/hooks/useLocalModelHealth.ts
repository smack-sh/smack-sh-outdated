...
export interface UseLocalModelHealthReturn {
  ...
  getHealthStatus: (provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string) => ModelHealthStatus | undefined;
  startMonitoring: (provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string, checkInterval?: number) => void;
  stopMonitoring: (provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string) => void;
  performHealthCheck: (provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string) => Promise<void>;
  isHealthy: (provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string) => boolean;
...
export function useLocalModelHealth(options: UseLocalModelHealthOptions = {}): UseLocalModelHealthReturn {
  ...
  const getHealthStatus = useCallback((provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string) => {
    return localModelHealthMonitor.getHealthStatus(provider, baseUrl);
  }, []);

  // Start monitoring a provider
  const startMonitoring = useCallback(
    (provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string, interval?: number) => {
  ...
  const stopMonitoring = useCallback((provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string) => {
  ...
  const performHealthCheck = useCallback(async (provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string) => {
    await localModelHealthMonitor.performHealthCheck(provider, baseUrl);
  }, []);

  // Check if a provider is healthy
  const isHealthy = useCallback(
    (provider: 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl: string) => {
...
export function useProviderHealth(
  provider: 'Ollama' | 'LMStudio' | 'geminiLike',
  baseUrl: string,
