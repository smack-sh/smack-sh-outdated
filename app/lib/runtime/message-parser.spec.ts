...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out smack artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <bol', 'Foo bar '],
      ['Foo bar <smack', 'Foo bar '],
      ['Foo bar <smacka', 'Foo bar <smacka'],
      ['Foo bar <smackA', 'Foo bar '],
      ['Foo bar <smackArtifacs></smackArtifact>', 'Foo bar <smackArtifacs></smackArtifact>'],
      ['Before <oltArtfiact>foo</smackArtifact> After', 'Before <oltArtfiact>foo</smackArtifact> After'],
      ['Before <smackArtifactt>foo</smackArtifact> After', 'Before <smackArtifactt>foo</smackArtifact> After'],
    ])('should correctly parse chunks and strip out smack artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <smackArtifact title="Some title" id="artifact_1">foo bar</smackArtifact> Some more text',
        {
      ...
      [
        [
          'Some text before <smackArti',
          'fact',
          ' title="Some title" id="artifact_1" type="bundled" >foo</smackArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        [
          ...
          ' ',
          '>',
          'foo</smackArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        [
          ...
          ' >fo',
          'o</smackArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <smackArtifact title="Some title" id="artifact_1">foo</smackArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <smackArtifact title="Some title" id="artifact_1"><smackAction type="shell">npm install</smackAction></smackArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <smackArtifact title="Some title" id="artifact_1"><smackAction type="shell">npm install</smackAction><smackAction type="file" filePath="index.js">some content</smackAction></smackArtifact> After',
        {
          output: 'Before  After',
