...
export class LocalModelHealthMonitor extends SimpleEventEmitter {
  ...
  private _checkIntervals = new Map<string, number>(); // Changed NodeJS.Timeout to number for browser compatibility
  ...
  private async _checkProviderHealth(
    ...
    try {
      switch (provider) {
        ...
        case 'geminiLike':
          return await this._checkgeminiLikeHealth(baseUrl, controller.signal);
        default:
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          responseTime: 0,
          error:
            'CORS_ERROR: LM Studio server is blocking cross-origin requests. Try enabling CORS in LM Studio settings or use smack desktop app.',
        };
      }
  ...
  /**
   * Check gemini-like provider health
   */
  private async _checkgeminiLikeHealth(baseUrl: string, signal: AbortSignal): Promise<HealthCheckResult> {
    try {
