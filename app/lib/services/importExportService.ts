...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          smack_user_profile: this._safeGetItem('smack_user_profile'),
          smack_settings: this._safeGetItem('smack_settings'),
          smack_profile: this._safeGetItem('smack_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('smack_viewed_features'),
          developer_mode: this._safeGetItem('smack_developer_mode'),
        ...
        ui: {
          // Tab configuration
          smack_tab_configuration: this._safeGetItem('smack_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('smack_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('smack_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          smack_read_logs: this._safeGetItem('smack_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('smack_last_acknowledged_version'),
        },
  ...
  static createAPIKeysTemplate(): Record<string, any> {
    ...
    return {
      _comment:
        "Fill in your API keys for each provider. Keys will be stored with the provider name (e.g., 'gemini'). The application also supports the older format with keys like 'gemini_API_KEY' for backward compatibility.",
      ...template,
    };
  }
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('smack_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.smack_tab_configuration) {
        try {
          this._safeSetItem('smack_tab_configuration', data.ui.smack_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'smack_acknowledged_debug_issues',
        'smack_acknowledged_connection_issue',
        'error_logs',
        'smack_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('smack_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
