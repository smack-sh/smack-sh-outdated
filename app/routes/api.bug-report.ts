...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      smackVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.smackVersion) {
      body += `- smack.sh: ${data.environmentInfo.smackVersion}\n`;
    }
  ...
  body += '---\n*Submitted via smack.sh bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/smack.sh';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'smack.sh-bug-reporter',
    });
