...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (!projectId || !token) {
    return json({ error: 'Missing projectId or admin token not configured' }, { status: 400 });
  }

  try {
    // Get project info using ADMIN token
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    // FORCE USE OF ADMIN TOKEN - Ignore any user-provided token
    const token = ADMIN_VERCEL_TOKEN;

    if (!token) {
      return json({ error: 'Admin Vercel token not configured' }, { status: 401 });
    }
    ...
    if (!targetProjectId) {
      const projectName = `smack-sh-${chatId}-${Date.now()}`;
