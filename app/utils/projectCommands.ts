...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<smackAction type="shell">${commands.setupCommand}</smackAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<smackAction type="start">${commands.startCommand}</smackAction>
`;
  }
...
export function escapesmackArtifactTags(input: string) {
  // Regular expression to match smackArtifact tags and their content
  const regex = /(<smackArtifact[^>]*>)([\s\S]*?)(<\/smackArtifact>)/g;
...
export function escapesmackAActionTags(input: string) {
  // Regular expression to match smackArtifact tags and their content
  const regex = /(<smackAction[^>]*>)([\s\S]*?)(<\/smackAction>)/g;
...
export function escapesmackTags(input: string) {
  return escapesmackArtifactTags(escapesmackAActionTags(input));
}
