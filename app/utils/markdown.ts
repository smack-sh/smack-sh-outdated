  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__smackThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__smackArtifact__', '__smackThought__', '__smackQuickAction', '__smackSelectedElement__'],

      // ['className', '__smackThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__smackArtifact__', '__smackThought__', '__smackQuickAction'],
    ],
  },
  strip: [],
};
