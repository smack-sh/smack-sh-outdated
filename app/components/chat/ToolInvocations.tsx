...
export const ToolInvocations = memo(({ toolInvocations, toolCallAnnotations, addToolResult }: ToolInvocationsProps) => {
  ...
  return (
    <div className="tool-invocation border border-smack-elements-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150">
      <div className="flex">
        <button
          className="flex items-stretch bg-smack-elements-background-depth-2 hover:bg-smack-elements-artifacts-backgroundHover w-full overflow-hidden"
          onClick={toggleDetails}
          aria-label={showDetails ? 'Collapse details' : 'Expand details'}
        >
          <div className="p-2.5">
            <div className="i-ph:wrench text-xl text-smack-elements-textSecondary hover:text-smack-elements-textPrimary transition-colors"></div>
          </div>
          <div className="p-2.5 w-full text-left">
            <div className="w-full text-smack-elements-textPrimary font-medium leading-5 text-sm">
              MCP Tool Invocations{' '}
              {hasToolResults && (
                <span className="w-full w-full text-smack-elements-textSecondary text-xs mt-0.5">
        ...
        <AnimatePresence>
          {hasToolResults && (
            <motion.button
              ...
              className="bg-smack-elements-artifacts-background hover:bg-smack-elements-artifacts-backgroundHover"
              onClick={toggleDetails}
            >
              <div className="p-2">
                <div
                  className={`${showDetails ? 'i-ph:caret-up-bold' : 'i-ph:caret-down-bold'} text-xl text-smack-elements-textSecondary hover:text-smack-elements-textPrimary transition-colors`}
                ></div>
              </div>
            </motion.button>
          )}
      ...
      <AnimatePresence>
        {hasToolCalls && (
          ...
          >
            <div className="bg-smack-elements-artifacts-borderColor h-[1px]" />

            <div className="px-3 py-3 text-left bg-smack-elements-background-depth-2">
              <ToolCallsList
        ...
        {hasToolResults && showDetails && (
          ...
          >
            ...
            <div className="p-5 text-left bg-smack-elements-actions-background">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 text-xs mb-1">
                {isErrorResult ? (
                  <div className="text-lg text-smack-elements-icon-error">
                    <div className="i-ph:x"></div>
                  </div>
                ) : (
                  <div className="text-lg text-smack-elements-icon-success">
                    <div className="i-ph:check"></div>
                  </div>
                )}
                <div className="text-smack-elements-textSecondary text-xs">Server:</div>
                <div className="text-smack-elements-textPrimary font-semibold">{annotation?.serverName}</div>
              </div>

              <div className="ml-6 mb-2">
                <div className="text-smack-elements-textSecondary text-xs mb-1">
                  Tool: <span className="text-smack-elements-textPrimary font-semibold">{toolName}</span>
                </div>
                <div className="text-smack-elements-textSecondary text-xs mb-1">
                  Description:{' '}
                  <span className="text-smack-elements-textPrimary font-semibold">{annotation?.toolDescription}</span>
                </div>
                <div className="text-smack-elements-textSecondary text-xs mb-1">Parameters:</div>
                ...
                <div className="text-smack-elements-textSecondary text-xs mt-3 mb-1">Result:</div>
                <div className="bg-[#FAFAFA] dark:bg-[#0A0A0A] p-3 rounded-md">
      ...
      <ul className="list-none space-y-4">
        {toolInvocations.map((tool, index) => {
          ...
          return (
            ...
            >
              <div className="bg-smack-elements-background-depth-3 rounded-lg p-2">
                <div key={toolCallId} className="flex gap-1">
                  <div className="flex flex-col items-center ">
                    <span className="mr-auto font-light font-normal text-md text-smack-elements-textPrimary rounded-md">
                      {toolName}
                    </span>
                    <span className="text-xs text-smack-elements-textSecondary font-light break-words max-w-64">
                  ...
                  <div className="flex items-center justify-end gap-2 ml-auto">
                    <button
                      className={classNames(
                        ...
                        'bg-transparent',
                        'text-smack-elements-textTertiary hover:text-smack-elements-textPrimary',
                    ...
                    <button
                      className={classNames(
                        ...
                        'text-accent-500 hover:text-smack-elements-textPrimary',
