  ...
  const images =
    parts?.filter(
      (part): part is FileUIPart =>
        part.type === 'file' && 'mimeType' in part && part.mimeType.startsWith('image/'),
    ) || [];
          ...
          {profile?.avatar || profile?.username ? (
            <div className="flex items-end gap-2">
              <img
                ...
                decoding="sync"
              />
              <span className="text-smack-elements-textPrimary text-sm">{profile?.username ?? ''}</span>
            </div>
          ) : (
    ...
    <div className="flex flex-col bg-accent-500/10 backdrop-blur-sm px-5 p-3.5 w-auto rounded-lg ml-auto">
      <div className="flex gap-3.5 mb-4">
        {images.map((item, index) => (
          <div
            key={index}
            className="relative flex rounded-lg border border-smack-elements-borderColor overflow-hidden"
          >
...
function stripMetadata(content: string) {
  ...
  const artifactRegex = /<smackArtifact\s+[^>]*>[\s\S]*?<\/smackArtifact>/gm;
