...
import { SiAmazon, SiGoogle, SiGithub, SiHuggingface, SiPerplexity, Sigemini } from 'react-icons/si';
...
const PROVIDER_DESCRIPTIONS: Partial<Record<ProviderName, string>> = {
  ...
  Github: 'Use gemini models hosted through GitHub infrastructure',
  gemini: 'Use GPT-4, GPT-3.5, and other gemini models',
};
...
const CloudProvidersTab = () => {
  ...
  useEffect(() => {
    const newFilteredProviders = Object.entries(settings.providers || {})
      .filter(([key]) => !['Ollama', 'LMStudio', 'geminiLike'].includes(key))
      .map(([key, value]) => ({
        name: key,
      ...
      >
        <div className="flex items-center justify-between gap-4 mt-8 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="text-md font-medium text-smack-elements-textPrimary">Cloud Providers</h4>
              <p className="text-sm text-smack-elements-textSecondary">Connect to cloud-based AI models and services</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-sm text-smack-elements-textSecondary">Enable All Cloud</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredProviders.map((provider, index) => (
            <motion.div
              key={provider.name}
              className={classNames(
                'rounded-lg border bg-smack-elements-background text-smack-elements-textPrimary shadow-sm',
            ...
            >
              <div className="absolute top-0 right-0 p-2 flex gap-1">
                {URL_CONFIGURABLE_PROVIDERS.includes(provider.name) && (
                  <motion.span
                    className="px-2 py-0.5 text-xs rounded-full bg-red-500/10 text-red-500 font-medium"
              ...
              <div className="flex items-start gap-4 p-4">
                <motion.div
                  className={classNames(
                    ...
                    'bg-smack-elements-background-depth-3 group-hover:bg-smack-elements-background-depth-4',
                    'transition-all duration-200',
                    provider.settings.enabled ? 'text-red-500' : 'text-smack-elements-textSecondary',
                  )}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-4 mb-2">
                    <div>
                      <h4 className="text-sm font-medium text-smack-elements-textPrimary group-hover:text-red-500 transition-colors">
                        {provider.name}
                      </h4>
                      <p className="text-xs text-smack-elements-textSecondary mt-0.5">
                        ...
                        {editingProvider === provider.name ? (
                          <input
                            ...
                            className={classNames(
                              ...
                              'bg-smack-elements-background-depth-3 border border-smack-elements-borderColor',
                          ...
                          >
                            <div className="flex items-center gap-2 text-smack-elements-textSecondary">
                              ...
                              <span className="group-hover/url:text-red-500 transition-colors">
