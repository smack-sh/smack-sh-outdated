...
export default function LocalProvidersTab() {
  ...
  const filteredProviders = useMemo(() => {
    return Object.entries(providers || {})
      .filter(([key]) => [...LOCAL_PROVIDERS, 'geminiLike'].includes(key))
      .map(([key, value]) => {
      ...
      .sort((a, b) => {
        // Custom sort: Ollama first, then LMStudio, then geminiLike
        const order = { Ollama: 0, LMStudio: 1, geminiLike: 2 };
  ...
  useEffect(() => {
    filteredProviders.forEach((provider) => {
      ...
      if (provider.settings.enabled && baseUrl) {
        ...
        startMonitoring(provider.name as 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl);
      } else if (!provider.settings.enabled && baseUrl) {
        console.log(`[LocalProvidersTab] Stopping monitoring for ${provider.name} at ${baseUrl}`);
        stopMonitoring(provider.name as 'Ollama' | 'LMStudio' | 'geminiLike', baseUrl);
      }
    });
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-red-500/20 to-blue-500/20 flex items-center justify-center ring-1 ring-red-500/30">
              <Cpu className="w-6 h-6 text-red-500" />
            </div>
            <div>
              <h2 className="text-2xl font-semibold text-smack-elements-textPrimary">Local AI Providers</h2>
              <p className="text-sm text-smack-elements-textSecondary">Configure and manage your local AI models</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-smack-elements-textSecondary">Enable All</span>
              <Switch
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className="bg-smack-elements-background-depth-2 hover:bg-smack-elements-background-depth-3 border-smack-elements-borderColor hover:border-red-500/30 transition-all duration-200 gap-2"
              >
          ...
          {filteredProviders.map((provider) => (
            <div key={provider.name} className="space-y-4">
              ...
              {provider.name === 'Ollama' && provider.settings.enabled && (
                <Card className="mt-4 bg-smack-elements-background-depth-2">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <PackageOpen className="w-5 h-5 text-red-500" />
                        <h3 className="text-lg font-semibold text-smack-elements-textPrimary">Installed Models</h3>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                        ...
                        className="bg-transparent hover:bg-smack-elements-background-depth-2"
                      >
                        {isLoadingModels ? (
                    ...
                    {isLoadingModels ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          className="bg-gradient-to-r from-red-500/8 to-red-600/8 hover:from-red-500/15 hover:to-red-600/15 border-red-500/25 hover:border-red-500/40 transition-all duration-300 gap-2 group shadow-sm hover:shadow-md font-medium"
                          _asChild
                        >
                          <a
                            href="https://ollama.com/library"
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-semibold text-smack-elements-textPrimary">Available Models</h3>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                    ...
                    {isLoadingLMStudioModels ? (
                      ...
                      <div className="grid gap-4">
                        {lmStudioModels.map((model) => (
                          <Card key={model.id} className="bg-smack-elements-background-depth-3">
                            <CardContent className="p-4">
                              <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                  <h4 className="text-sm font-medium text-smack-elements-textPrimary font-mono">
                                    {model.id}
                                  </h4>
        ...
        {filteredProviders.length === 0 && (
          <Card className="bg-smack-elements-background-depth-2">
            <CardContent className="p-8 text-center">
              <Server className="w-16 h-16 mx-auto text-smack-elements-textTertiary mb-4" />
              <h3 className="text-lg font-medium text-smack-elements-textPrimary mb-2">No Local Providers Available</h3>
